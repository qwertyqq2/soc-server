...
contract Group {
    ...
    function GetLionDota() external view returns(uint256){
        return LionDota;
    }
    function UpdateLionDota(uint256 data) external{
        LionDota = uint256(keccak256(abi.encode(data, LionDota)));
        emit LionDotaEvent(LionDota);
    }
